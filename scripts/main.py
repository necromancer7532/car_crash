###################################################################################################
#                                                                                                 #
#                                  Created By : Vishnu Remesh                                     #
#                                  Refer Usage.txt for how to run                                 #
#                                                                                                 #
###################################################################################################
import sys
import json
...
###################################################################################################################
    ...
    print("Question number not provided running all scenarios")
    config_for_question = config_data[0]
    file_read_obj = FileRead(spark)
    ...
    count_crashes = accident_count_obj.count_damages(df_dict)
    logger.info(
        "Count of Distinct Crash IDs where No Damaged Property was observed and Damage Level is above 4 and car avails Insurance : {}".format(
            count_crashes))
